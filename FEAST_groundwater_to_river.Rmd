...
feast_meta = read.csv("/FEAST_input/feast_input_groundwater_to_river.csv")
...
Run FEAST analysis 
```{r}
path="/feast_out_groundwater_to_river"
...
df_clean$Sink=gsub(".*_","",df_clean$Sink)
df_clean$Sources=gsub("\\.\\.","\\. ",df_clean$variable)
df_clean$Sources=gsub("y\\.","y",df_clean$Sources)
df_clean$Sources=gsub("e\\.","e",df_clean$Sources)

df_clean$Month=gsub(" river", "", df_clean$Sink) # Add column for months 
df_clean$Month=gsub("y\\.","y",df_clean$Month)
df_clean$Month=gsub("e\\.","e",df_clean$Month)

# Adding value for missing February and March river 
df_clean[nrow(df_clean)+1,]=list("Feb. river", "Missing data", 1, "Saint-Lazare", "Missing data", "Feb.") # Adding missing February for Saint-Lazare
df_clean[nrow(df_clean)+1,]=list("Feb. river", "Missing data", 1, "Rigaud", "Missing data", "Feb.") # Adding missing February for Rigaud 
df_clean[nrow(df_clean)+1,]=list("Mar. river", "Missing data", 1, "Rigaud", "Missing data", "Mar.")# Adding missing March for Rigaud 


# Order sources and months as a factor
df_clean$Sources=factor(df_clean$Sources, levels=c("Missing data", "Unknown", "Sep. groundwater","Oct. groundwater","Nov. groundwater",
                                                   "Dec. groundwater","Jan. groundwater","Feb. groundwater","Mar. groundwater","Apr. groundwater",
                                                   "May groundwater","June groundwater","July groundwater","Nov. snow","Jan. snow", "Feb. snow", 
                                                   "Mar. snow","Sep. river","Oct. river","Nov. river","Dec. river","Jan. river","Mar. river",
                                                   "Apr. river","May river","June river","July river")) 
...
colors=read.csv("color_groundwater_to_river.csv", header=FALSE, row.names = 1)
tcolors=t(colors)
...
r_plot_feast=ggplot(R, aes(x = Month, fill=Sources, y=value)) + 
  ...
  my_scale1 + 
  guides(fill=guide_legend(nrow=6, title="Sources", title.position="top", title.hjust = 0.55))
...
sl_plot_feast=ggplot(sl, aes(x = Month, fill=Sources, y=value)) + 
  ...
  geom_bar(stat="identity") +
  coord_flip() +
  labs(title="Saint-Lazare", y="", x= "Sink (river)") + 
  theme(text = element_text(size=12),
        legend.justification = "center",
        legend.position="bottom", 
        plot.title = element_text(hjust =0.5, size=12),
...
Save plot 
```{r}
ggsave("feast_groundwater_to_river.pdf",plot=combined_plot, device="pdf", path="/image/", width=16.5, height = 16.5, units = "cm")
```
...
write.table(R, "/FEAST_ouput_combined/FEAST_out_Rigaud_river.csv", quote=FALSE, sep=",", row.names = FALSE)
write.table(sl, "/FEAST_ouput_combined/FEAST_out_SaintLaz_river.csv", quote=FALSE, sep=",", row.names = FALSE)
```
